...
public class cryptoText extends PApplet {
...
public int getSeed() {  
  ...
  for (int i = 0; i < thisKey.length()-1; i++) 
    keySeed *= PApplet.parseInt(thisKey.charAt(i) * (thisKey.charAt(i)-thisKey.charAt(i+1)));  // перемножением с разностью
  return keySeed;
}
  ...
  if (refPath.length() != 0 && textPath.length() != 0) {
    ...
    for (int i = 0; i < lines.length; i++) {         // пробегаем по строкам
      for (int j = 0; j < lines[i].length() + 1; j++) {  // и каждому символу в них +1
        ...
        int thisChar;
        if (j == lines[i].length()) thisChar = PApplet.parseInt('\n');  // последний - перенос строки
...
public void ImageIsNotSelected() {                      // Штука чтобы делать перевод этих строк.
if (eng == false) {
...
public void EmptyTextFile() {                      // Штука чтобы делать перевод этих строк.
if (eng == false) {
...
public void ImageIsTooSmall() {                      // Штука чтобы делать перевод этих строк.
if (eng == false) {
...
public void finished() {                      // Штука чтобы делать перевод этих строк.
if (eng == false) {
...
public void decrypt() {
  ...
  if (cryptPath.length() != 0) {
    ...
    while (true) {
      ...
      decryptText += PApplet.parseChar(thisChar);            // пишем в буфер
    }
...
public void CryptedImageIsNotSelected() {                      // Штука чтобы делать перевод этих строк.
if (eng == false) {
...
// прочие кнопки
public void load_ref() {
  selectInput("", "selectRef");
}

public void selectRef(File selection) {
  if (selection != null) {
...
public void load_crypt() {
  selectInput("", "selectCrypt");
}

public void selectCrypt(File selection) {
  if (selection != null) {
...
public void load_crypt_text() {
  selectInput("", "selectCryptText");
}

public void selectCryptText(File selection) {
  if (selection != null) {
...
public void TextFileIsNotSelected() {                      // Штука чтобы делать перевод этих строк.
if (eng == false) {
...
public void rus() {
  ...
  public void settings() {  size(500, 280); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "cryptoText" };
    if (passedArgs != null) {
